...
import { formatMinutesSeconds } from '@/components/games/crossword/standings'

// Mirrors web CrosswordGameTimerBar: a "Puzzle time left" countdown bar driven by the
...
export function CrosswordGameTimerBar({
  ...
  useEffect(() => {
    ...
    const fire = async () => {
      try {
        await fetch(apiUrl(`/api/games/${gameCode.toUpperCase()}/expire-crossword`), { method: 'POST' })
      } catch {
