...
/**
 * Build a shareable PGN from a finished (or in-progress) chess session. Ported
 * from src/lib/chess-pgn.ts (web). The movetext is already maintained in
 * `session.pgn`; this wraps it with the Seven Tag Roster + derived tags so it
 * can be imported into Lichess, chess.com, or any PGN reader.
 */
...
function timeControl(game: Game): string | null {
  const seconds = game.timer_seconds ?? 0
  return seconds > 0 ? String(seconds) : null
}

export type ChessPgnExport = {
...
/** A filesystem-safe `.pgn` filename, e.g. `alice-vs-bob.pgn`. */
