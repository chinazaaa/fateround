import type { CheckersColor, Draughts10Session } from '@fateround/shared'

/** True when the match is timed (both players have a clock budget). */
export function draughts10IsTimed(session: Draughts10Session): boolean {
 ...
 */
export function liveDraughts10ClockMs(session: Draughts10Session, color: CheckersColor): number {
...
export function formatDraughts10Clock(ms: number): string {
...
export function draughts10ResultDetail(reason: string | null): string | null {
  switch (reason) {
    case 'capture_all':
