...
/**
 * Bespoke Checkers final-results card — the mobile counterpart of the web
 * Checkers results share block. Self-contained: its own header (room title +
 * CHECKERS label), a winner/draw hero, and the existing CheckersFinalBoard
 * (matchup + 8×8 final position + result detail) as the body. Includes a button
 * that snapshots the visible card to a PNG for image sharing.
 *
 * CheckersFinalBoard already renders the result-reason detail, so `reasonSubtitle`
 * is intentionally not shown in the hero to avoid duplicating it.
 */
export function CheckersShareCard({
  gameTitle,
  winnerName,
  isDraw,
    ...
    <View style={styles.wrap}>
      ...
      <View ref={captureCardRef} collapsable={false} style={styles.card}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerLabel}>CHECKERS</Text>
        </View>
        ...
        <View style={styles.body}>
          <CheckersFinalBoard session={session} players={players} highlightPlayerId={highlightPlayerId} />
        </View>
      </View>
