...
import { formatMinutesSeconds } from '@/components/games/sudoku/standings'

// Mirrors web SudokuGameTimerBar: a "Puzzle time left" countdown bar driven by the
// host's max-time-limit (game_duration_seconds). Turns urgent (red) under 60s and
// best-effort fires the server expiry once the clock hits zero.
export function SudokuGameTimerBar({
  gameCode,
  game,
  onExpired,
}: {
  ...
  useEffect(() => {
    ...
    const fire = async () => {
      try {
        await fetch(apiUrl(`/api/games/${gameCode.toUpperCase()}/expire-sudoku`), { method: 'POST' })
      } catch {
  ...
  return (
    <View style={[styles.bar, urgent && styles.barUrgent]}>
      <View style={styles.row}>
        <Text style={styles.label}>Puzzle time left</Text>
...
const makeStyles = (theme: Theme) =>
  StyleSheet.create({
    ...
    // Functional urgent-red state — kept fixed across themes.
