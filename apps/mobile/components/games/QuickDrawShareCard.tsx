...
import { teamLabel } from '@fateround/shared/quick-draw-guess'
import type { DescribeItPlayerScore, DescribeItTeamScore } from '@fateround/shared/describe-it'
...
type TeamModeProps = {
  mode: 'team'
  /** Ranked team scores from computeQuickDrawGuessScores. */
  teamScores: DescribeItTeamScore[]
  /** Winning team numbers (empty when nothing guessed). */
  winners: number[]
...
type IndividualModeProps = {
  mode: 'individual'
  /** Ranked players from quickDrawGuessIndividualLeaderboard. */
...
/**
 * Bespoke Quick Draw final-results card — the mobile counterpart of the web
 * `QuickDrawGuessFinishedResults` share block (which itself reuses the Describe
 * It capture card, since guess-mode mirrors Describe It). Renders a theme-aware
 * winner hero plus ranked standings (team scores with team badges + word
 * counts, or an individual points leaderboard), and a dedicated Share button
 * that snapshots a fixed light-themed copy to a PNG for image sharing (text
 * fallback if capture or share fails).
 */
export function QuickDrawShareCard(props: Props) {
    ...
    <View style={styles.wrap}>
      ...
      <View style={styles.offscreen} pointerEvents="none">
        <View ref={captureCardRef} collapsable={false} style={styles.captureCard}>
          ...
          <Text style={styles.captureGame}>Quick Draw</Text>
