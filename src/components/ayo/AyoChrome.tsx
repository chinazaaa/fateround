...
export function AyoCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`glass-card ${className}`}>{children}</div>
}

export function AyoSecondaryButton({
  children,
  onClick,
  disabled,
  className = '',
...
export function AyoLoadingScreen() {
  return (
    <AyoShell>
      <AyoCard className="p-8 text-center">
...
export function AyoTurnBar({ turnPlayerName, isMyTurn }: { turnPlayerName?: string; isMyTurn?: boolean }) {
  return (
    <div
      className={[
    ...
    >
      <span>{isMyTurn ? 'Your turn — pick a house' : turnPlayerName ? `${turnPlayerName}'s turn` : 'Waiting…'}</span>
    </div>
  )
}
