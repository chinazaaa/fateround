...
import { useGoFishGameTimer } from '@/hooks/useGoFishGameTimer'
import type { Game } from '@/types'

/**
 * Whole-game countdown. Mirrors WhotGameTimerBar shape so both card games share the
 * same clock chrome. Sticky at the top of the round via a wrapper in the parent — see
 * GoFishActiveRound — so a scrolled-down player still sees how long they have.
 */
export function GoFishGameTimerBar({
  gameCode,
  game,
}: {
  gameCode: string
  ...
  const { active, label, secondsLeft, durationSeconds } = useGoFishGameTimer(gameCode, game)
  ...
  return (
    ...
    >
      ...
      <p className="mt-1.5 text-[10px] text-muted text-center">When time runs out, most books wins</p>
    </div>
  )
}
