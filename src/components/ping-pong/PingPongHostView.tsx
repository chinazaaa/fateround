...
import { HostPingPongLobbyPanel } from '@/components/host-lobby/HostPingPongLobbyPanel'
import { TransferHostControl } from '@/components/TransferHostControl'
import { HostEndGameButton } from '@/components/ui/HostEndGameButton'
import { ExitIcon } from '@/components/host/host-icons'
import { lobbyMaxPlayersFromGameClient } from '@/lib/game-limits'
import { gameTypeConfig } from '@/lib/game-types'
import { PING_PONG_MIN_PLAYERS, isPingPongResultsPhase } from '@/lib/ping-pong'
import { supabase } from '@/lib/supabase'
import { GAME_SELECT, PLAYER_SELECT, PING_PONG_SESSION_SELECT } from '@/lib/supabase-selects'
...
type PingPongHostMode = 'spectator' | 'player'

const HOST_MODE_KEY = 'ping_pong_host_mode'

function getHostMode(gameCode: string): PingPongHostMode {
  if (typeof window === 'undefined') return 'player'
  try {
    return (localStorage.getItem(`${HOST_MODE_KEY}_${gameCode}`) as PingPongHostMode) ?? 'player'
  } catch {
    return 'player'
  }
}

function setHostMode(gameCode: string, mode: PingPongHostMode): void {
...
export function PingPongHostView({ gameCode, hostToken }: { gameCode: string; hostToken: string }) {
  ...
  const [hostMode, setHostModeState] = useState<PingPongHostMode>('player')
  ...
  const load = useCallback(async (): Promise<boolean> => {
    ...
    if (supabasePollOk(sessionRes)) {
      setSession(sessionRes.data as PingPongSession | null)
    }
  ...
  useEffect(() => {
    if (isPingPongResultsPhase(game?.status, session)) setTab('manage')
  ...
  const changeHostMode = async (mode: PingPongHostMode) => {
    const prev = hostMode
  ...
  const canStart = readyPlayers.length >= PING_PONG_MIN_PLAYERS
  const winner = players.find((p) => p.id === session?.winner_player_id)
  const hostPlays = hostMode === 'player' && !!hostPlayerId
  const gameFinished = isPingPongResultsPhase(game?.status, session)
            ...
            startDisabledHint={
              canStart
                ...
                : readyPlayers.length < players.length
                  ? `Waiting for players to tap ready (${readyPlayers.length}/${PING_PONG_MIN_PLAYERS})`
                  : `Need exactly ${PING_PONG_MIN_PLAYERS} players to start (${players.length}/${PING_PONG_MIN_PLAYERS})`
            }
            className="space-y-3"
          />
  ...
  if (waitingLobby) {
    return (
      <HostLobby
        ...
        settingsChildren={
          <>
            <HostPingPongLobbyPanel gameCode={gameCode} hostToken={hostToken} game={game} onGameUpdate={setGame} />
