          ...
          {(title || subtitle) && !compact && (
            <header className="text-center space-y-2">
              <div className="inline-flex items-center justify-center gap-2">
                ...
                <GameTypeBadge gameType="ping_pong" />
              </div>
              {title && (
...
export function PingPongCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`glass-card ${className}`}>{children}</div>
}

export function PingPongPrimaryButton({
  children,
  onClick,
  disabled,
  loading,
...
export function PingPongSecondaryButton({
  children,
  onClick,
  disabled,
  className = '',
...
export function PingPongLoadingScreen() {
  return (
    <PingPongShell>
      <PingPongCard className="p-8 text-center">
