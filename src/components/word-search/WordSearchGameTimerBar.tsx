...
/**
 * Countdown bar for the shared word-search race. Mirrors CrosswordGameTimerBar: a duration of
...
export function WordSearchGameTimerBar({
  ...
  useEffect(() => {
    ...
    const fire = async () => {
      try {
        await fetch(`/api/games/${gameCode}/expire-word-search`, { method: 'POST' })
      } catch {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted">Hunt time left</p>
