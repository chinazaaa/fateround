...
export function CheckersHostView({ gameCode, hostToken }: { gameCode: string; hostToken: string }) {
  ...
  const changeHostMode = async (mode: CheckersHostMode) => {
    const prev = hostMode
  ...
  if (waitingLobby) {
    return (
      <HostLobby
        ...
        hostToken={hostToken}
        game={game}
        gameTypeLabel={gameTypeConfig('checkers').label}
        players={players}
        maxPlayers={lobbyMaxPlayersFromGameClient('checkers', game) ?? game.max_players}
        playCard={
          <HostModeSelector
            mode={hostMode}
            onChange={changeHostMode}
