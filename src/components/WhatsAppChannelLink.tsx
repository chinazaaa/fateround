...
const WhatsAppIcon = ({ size = 14 }: { size?: number }) => (
  <svg viewBox="0 0 24 24" fill="currentColor" width={size} height={size} className="shrink-0" aria-hidden>
...
export function WhatsAppChannelLink({ className = '', children }: { className?: string; children?: React.ReactNode }) {
  const url = useCommunityLink()
  return (
    <a
      href={url}
      target="_blank"
      rel="noopener noreferrer"
      className={`inline-flex items-center gap-1.5 rounded-full border border-[#25D366]/40 px-3 py-1.5 text-xs font-semibold text-[#1a9e4e] transition-colors hover:bg-[#25D366]/10 dark:text-[#25D366] dark:hover:bg-[#25D366]/10 ${className}`}
    >
