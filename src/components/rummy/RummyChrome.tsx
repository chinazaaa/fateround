...
/**
 * Shared page shell for every Rummy screen — mirrors the TicTacToeShell / CrazyEightsShell
 * layout, with a Rummy header (emoji + label) and centred, width-capped content.
 */
export function RummyShell({
  children,
  title,
  subtitle,
  wide,
...
export function RummyCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`glass-card ${className}`}>{children}</div>
}

export function RummyPrimaryButton({
  children,
  onClick,
  disabled,
  loading,
...
export function RummySecondaryButton({
  children,
  onClick,
  disabled,
  className = '',
...
export function RummyLoadingScreen() {
  return (
    <RummyShell>
      <RummyCard className="p-8 text-center">
...
/** Colour token for a suit label / suit symbol — hearts/diamonds red, others neutral. */
export function suitColorClass(suit: string): string {
  return suit === 'hearts' || suit === 'diamonds' ? 'text-red-400' : 'text-neutral-100'
}
