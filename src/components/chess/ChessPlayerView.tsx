...
export function ChessPlayerView({ gameCode }: { gameCode: string }) {
  ...
  // row changes. For a live clock game that gap is catastrophic: a player waiting to
  ...
  // tight 4s duel cadence when the channel is actually down); the poll's acceptSession
  // funnel keeps whichever row has the newest updated_at, so a healthy realtime push
  // still wins and this only corrects a stuck view.
