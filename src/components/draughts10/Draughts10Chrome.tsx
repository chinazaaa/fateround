...
export function Draughts10Shell({
  gameType,
  children,
  title,
  subtitle,
...
export function Draughts10Card({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`glass-card ${className}`}>{children}</div>
}

export function Draughts10PrimaryButton({
  children,
  onClick,
  disabled,
  loading,
...
export function Draughts10SecondaryButton({
  children,
  onClick,
  disabled,
  className = '',
...
export function Draughts10LoadingScreen({ gameType }: { gameType: GameType }) {
  return (
    <Draughts10Shell gameType={gameType}>
      <Draughts10Card className="p-8 text-center">
...
export function Draughts10TurnBar({
  turnPlayerName,
  isMyTurn,
  mustContinue,
}: {
  ...
  return (
    ...
    >
      <span>
        ...
        {mustContinue && <span className="ml-1.5 text-amber-400 font-black">Keep jumping!</span>}
      </span>
    </div>
  )
}
