...
import { currentTurnPlayerId, DRAUGHTS10_MIN_PLAYERS, isDraughts10ResultsPhase } from '@/lib/draughts10'
import { supabase } from '@/lib/supabase'
import { GAME_SELECT, PLAYER_SELECT, DRAUGHTS10_SESSION_SELECT } from '@/lib/supabase-selects'
import { useHostAutoReady } from '@/hooks/useHostAutoReady'
import { useHostRemovePlayer } from '@/hooks/useHostRemovePlayer'
import { useHostSeat } from '@/hooks/useHostSeat'
import type { Game, Player, GameType, Draughts10Session } from '@/types'
 ...
 * game's own game_type (checkers_international vs checkers_nigeria). */
...
export function Draughts10HostView({ gameCode, hostToken }: { gameCode: string; hostToken: string }) {
  ...
  const load = useCallback(async (): Promise<boolean> => {
    ...
    if (supabasePollOk(sessionRes)) {
      setSession(sessionRes.data as Draughts10Session | null)
    }
  ...
  const resolvedGameType: GameType = (game?.game_type as GameType) ?? 'checkers_international'
  const apiBase = apiBaseForGameType(game?.game_type)

  // Land on the primary (Play/Watch) tab when the game starts, and on Manage at results.
  useEffect(() => {
    if (isDraughts10ResultsPhase(game?.status, session)) setTab('manage')
  ...
  // Reconciliation poll — mirrors chess. This game (checkers international / nigeria) has a
  // cumulative clock that LOSES the game on time, so a client stranded on a stale `current_turn`
  // (ticking the wrong clock, never seeing it's its turn) is catastrophic. `connected` can't detect
  // a channel that stays subscribed but silently stops delivering rows, so during an active match
  // we ALWAYS poll: a slow 10s safety reconcile while connected, the tight 4s duel cadence when the
  ...
  const canStart = readyPlayers.length >= DRAUGHTS10_MIN_PLAYERS
  const turnPlayerId = session ? currentTurnPlayerId(session) : null
  const winner = players.find((p) => p.id === session?.winner_player_id)
  const hostPlays = hostMode === 'player' && !!hostPlayerId
  const gameFinished = isDraughts10ResultsPhase(game?.status, session)
  const isHostTurn = turnPlayerId === hostPlayerId

  useDraughts10ClockExpiry(gameCode, session, game?.status === 'active', apiBase)
            ...
            startDisabledHint={
              canStart
                ...
                : readyPlayers.length < players.length
                  ? `Waiting for players to tap ready (${readyPlayers.length}/${DRAUGHTS10_MIN_PLAYERS})`
                  : `Need exactly ${DRAUGHTS10_MIN_PLAYERS} players to start (${players.length}/${DRAUGHTS10_MIN_PLAYERS})`
            }
            className="space-y-3"
          />
        ) : game.status === 'active' && !gameFinished ? (
  ...
  if (waitingLobby) {
    return (
      <HostLobby
        ...
        hostToken={hostToken}
        game={game}
        gameTypeLabel={gameTypeConfig(resolvedGameType).label}
        titleMeta={<GameInfoChips game={game} className="mt-2" />}
        players={players}
        maxPlayers={lobbyMaxPlayersFromGameClient(resolvedGameType, game) ?? game.max_players}
        playCard={
          <HostModeSelector
            mode={hostMode}
            onChange={changeHostMode}
        ...
        settingsChildren={
          <>
            <HostDuelLobbyPanel
              ...
              hostToken={hostToken}
              game={game}
              duelType={resolvedGameType === 'checkers_nigeria' ? 'checkers_nigeria' : 'checkers_international'}
              onGameUpdate={setGame}
            />
