...
export function MahjongShell({
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      >
        <div
          className={['w-full', compact ? 'space-y-2' : 'space-y-5 sm:space-y-6', wide ? 'max-w-4xl' : 'max-w-lg'].join(
            ' '
          )}
        >
          {(title || subtitle) && !compact && (
            <header className="text-center space-y-2">
              <div className="inline-flex items-center justify-center gap-2">
                ...
                <GameTypeBadge gameType="mahjong" />
              </div>
              {title && (
...
export function MahjongCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`glass-card ${className}`}>{children}</div>
}

export function MahjongPrimaryButton({
  children,
  onClick,
  disabled,
  loading,
...
export function MahjongSecondaryButton({
  children,
  onClick,
  disabled,
  className = '',
...
export function MahjongLoadingScreen() {
  return (
    <MahjongShell>
      <MahjongCard className="p-8 text-center">
        <p className="text-muted animate-pulse">Loading table...</p>
      </MahjongCard>
    </MahjongShell>
  )
}

export function MahjongTurnBar({
  turnPlayerName,
  isMyTurn,
  phase,
  secondsLeft,
  ...
  const phaseLabel = phase === 'claim' ? 'Calls open' : phase === 'draw' ? 'Drawing tile' : 'Choose a discard'
      ...
      <span>
        {phase === 'claim'
          ...
          : isMyTurn
            ...
            : turnPlayerName
              ? `${turnPlayerName}'s turn - ${phaseLabel}`
              : 'Waiting...'}
      </span>
