 ...
 * their own isolated clock-chip components, keeping moves smooth and the
 ...
 *
 * `apiBase` is the variant's API route base (`/api/checkers-international` or
 * `/api/checkers-nigeria`) — Draughts10 is shared by both game types but each has
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      if (remaining <= 0 && !expiringRef.current) {
        ...
        void fetch(`${apiBase}/expire-turn`, {
          method: 'POST',
