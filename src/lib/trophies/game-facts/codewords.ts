 ...
 *
 * TEAM WINS ARE NOT PERSONAL WINS. `resolveWinners` returns the whole winning team, so `opts.won`
 * is true for an operative who never guessed and for a spymaster whose partner did all the work.
 * Every `_wins` counter below is therefore a TEAM property credited to each member of the team;
 * that is deliberate (Codewords has no individual win) and each one says so where it matters.
 * The personal counters — the guess tallies and the run counters — are the only ones that measure
 * this player specifically.
 *
...
export async function codewordsFacts(
  ...
  const [{ data: boards }, { data: roles }, { data: guesses }] = await Promise.all([
    supabase
      ...
      // `boards.winner` is deliberately not read: `opts.won` is the award pass's own verdict via
  ...
  // A player with no role row and no guesses was never in this game (a spectator, or a row that
  // never got written). Nothing to say about them — and never a throw.
  const myTeam = myRole?.team ?? myGuesses[0]?.team ?? null
  if (!myTeam) return facts
  ...
  for (const run of myRuns) {
    ...
    if (n >= 2 && n <= 5 && found >= n) facts[`codewords_clue${n}_full`] = 1
  ...
  const cleanTurns = myRuns.filter((r) => r.guesses.length > 0 && r.guesses.every((g) => g.cell_type === myTeam)).length
  if (cleanTurns) facts.codewords_clean_turns = cleanTurns

  if (!opts.won) return facts
  ...
  if (!assassinHit) facts.codewords_assassin_dodged_wins = 1

  const myTeamGuesses = allGuesses.filter((g) => g.team === myTeam)
  const wrong = myTeamGuesses.filter((g) => g.cell_type !== myTeam).length
  if (wrong === 0) facts.codewords_perfect_wins = 1
  if (myRuns.length > 0 && myRuns.length <= SWEEP_RUNS) facts.codewords_sweep_wins = 1
  if (myRuns.length > 0 && myRuns.length <= FLAWLESS_SWEEP_RUNS && wrong === 0) facts.codewords_flawless_sweep_wins = 1

  // ── Key-derived win shapes ────────────────────────────────────────────────────────────
  const key = board?.key ?? null
  if (!key?.length) return facts
  ...
  if (remaining(opponentTeam, finalRevealed) === CLUTCH_OPPONENT_REMAINING) facts.codewords_clutch_wins = 1
  ...
  // The starting team begins one card down by design, so a deficit of 1 is the normal opening and
  ...
  if (worstDeficit >= COMEBACK_DEFICIT) facts.codewords_comeback_wins = 1
