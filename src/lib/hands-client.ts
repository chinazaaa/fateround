import type { BingoCard, GoFishPlayerHand, UnoPlayerHand, WhotPlayerHand } from '@/types'

/** Games whose per-player hand table is read through a redaction route (`/api/<game>/hands`). */
export type HandsGame = 'whot' | 'uno' | 'gofish'
 ...
 * fixes to them, can only ever exist once — Crazy Eights joins by passing its name.
 */
...
/** Go Fish hands — see {@link fetchHands}. */
export function fetchGoFishHands(
  gameCode: string,
  auth: { resumeToken?: string | null; hostToken?: string | null }
): Promise<GoFishPlayerHand[] | null> {
  return fetchHands<GoFishPlayerHand>('gofish', gameCode, auth)
}
