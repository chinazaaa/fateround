...
import { admitCrazyEightsPlayer } from './crazy-eights'
import type { CrazyEightsCard } from '@/types'

// admitCrazyEightsPlayer seats a spectator into a live Crazy Eights game: append to
// turn_order, deal a hand from the draw pile, flip spectator=false — all guarded so cards are
// never duplicated and went-out/finished players can't be re-dealt-in. These tests pin the CAS
// ordering (claim the session BEFORE materializing the hand), the deal count, the reshuffle
// path, every rejection guard, the retry-on-lost-race behavior, and that the current player's
// turn/clock AND the play direction are left untouched. Mirrors whot-admit.test.ts.

type Row = { data: unknown; error: unknown }

function cards(n: number, prefix = 'c'): CrazyEightsCard[] {
  return Array.from({ length: n }, (_, i) => ({ id: `${prefix}-${i}`, suit: 'spades', rank: (i % 13) + 1 }))
}
    ...
    from(table: string) {
      return {
        ...
        update(vals: Record<string, unknown>) {
          if (table === 'crazy_eights_sessions') {
            // Forward CAS claims are gated by casWins; the compensating rollback is a later
            // crazy_eights_sessions.update with no casWins entry left, so it just resolves.
        ...
        insert(vals: Record<string, unknown>) {
          if (table === 'crazy_eights_player_hands') ops.handInserts.push(vals)
          return chain({
            data: null,
            error: table === 'crazy_eights_player_hands' ? (opts.handError ?? null) : null,
          })
        },
        delete() {
          if (table === 'crazy_eights_player_hands') ops.handDeletes += 1
          return chain({ data: null, error: null })
        },
      }
    },
...
function baseSession(over: Record<string, unknown> = {}) {
  return {
    ...
    discard_pile: [],
    top_card: { id: 'spades-3', suit: 'spades', rank: 3 },
...
describe('admitCrazyEightsPlayer', () => {
  it('seats the spectator, deals 5, flips spectator — and never touches the turn clock or direction', async () => {
    const m = makeMock({ session: baseSession(), player: latecomer, casWins: [true] })
    const res = await admitCrazyEightsPlayer(m.supabase, 'G1', 'D', 6)

    expect(res).toEqual({ error: null, status: 200 })
    const patch = m.ops.sessionUpdates[0]
    expect(patch.turn_order).toEqual(['A', 'B', 'C', 'D'])
    expect((patch.draw_pile as CrazyEightsCard[]).length).toBe(5) // 10 − 5 dealt
    // The current player's turn + countdown + play direction must be preserved.
    expect('current_turn_index' in patch).toBe(false)
    expect('turn_deadline_at' in patch).toBe(false)
    expect('direction' in patch).toBe(false)

    expect(m.ops.handInserts).toHaveLength(1)
    expect(m.ops.handInserts[0].player_id).toBe('D')
    expect((m.ops.handInserts[0].cards as CrazyEightsCard[]).length).toBe(5)
  ...
  it('deals 5 even when admitting into a heads-up (2→3) table', async () => {
    ...
    await admitCrazyEightsPlayer(m.supabase, 'G1', 'D', 6)
    expect((m.ops.handInserts[0].cards as CrazyEightsCard[]).length).toBe(5)
  })
  ...
  it('blocks while the board is mid-choice (phase !== playing) — no writes', async () => {
    for (const phase of ['choose_suit', 'finished']) {
  ...
  it('rolls back the seat when the hand insert fails (500) — turn_order + deck restored', async () => {
    ...
    expect(m.ops.sessionUpdates).toHaveLength(2) // forward claim + compensating rollback
    expect(m.ops.sessionUpdates[1].turn_order).toEqual(['A', 'B', 'C']) // seat undone
    expect((m.ops.sessionUpdates[1].draw_pile as CrazyEightsCard[]).length).toBe(10) // full deck restored
