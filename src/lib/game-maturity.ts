 ...
 */
const CONSENT_STORAGE_KEY = 'fr:mature-ack'

export function hasAcknowledgedMature(): boolean {
  if (typeof window === 'undefined') return false
  try {
    return window.localStorage.getItem(CONSENT_STORAGE_KEY) === '1'
  } catch {
    // Safari private mode and blocked-storage setups throw on access — fail open rather than
    // trapping the player behind a gate they can never clear.
    return true
  }
}

export function acknowledgeMature(): void {
