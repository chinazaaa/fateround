 ...
 *   1. Capture  — always take a capture; the disruption is worth more than
 *                 raw progress. Two captures in one move (multi-victim square)
 *                 scores higher than one.
 *   2. Finish   — landing a piece on the finish square is a permanent win
 *                 toward the game outcome; take it.
 *   3. Home     — moving into the home lane is progress no opponent can
 *                 disrupt (opponents can't enter my lane).
 *   4. Bring-out — get a piece out of base when a 6 offers it, so I have
 *                 more pieces to work with next turn.
 *   5. Furthest — otherwise, prefer the move that advances the piece
 *                 currently furthest along the track (leading pieces are
 *                 closer to finishing; also concentrates development).
 *
...
import { HOME_ENTRY_STEPS, START_POS, TRACK_LENGTH, type LudoMoveOption } from '@/lib/ludo'
...
function scoreMove(move: LudoMoveOption, color: LudoColor): number {
 ...
 */
export function pickLudoBotMove(moves: LudoMoveOption[], botPlayerState: LudoPlayerState): LudoMoveOption | null {
