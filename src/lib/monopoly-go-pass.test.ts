...
describe('monopoly GO pass & cash accumulation', () => {
  it('awards GO salary when wrapping around the board', () => {
    const move = movePosition(38, 4) // moves 4 steps from 38 to 2 (Community Chest / Esusu Fund)
  ...
  it('does not award GO salary when moving away from space 0 without wrapping', () => {
    const move = movePosition(0, 2) // rolls 2 from GO to Community Chest
    expect(move.to).toBe(2)
    expect(move.passedGo).toBe(false)
  })

  it('integration: preserves GO salary when landing on Community Chest / Esusu Fund space actions', () => {
    const states = [
      {
        game_id: 'game-1',
        player_id: 'player-1',
    ...
    // Wrapping movement: 38 -> 2 (Community Chest / Esusu Fund)
    ...
    // Non-wrapping movement: 0 -> 2 (Community Chest / Esusu Fund)
