...
import { processUnoExpireTurn } from '@/lib/uno'
import { unoActionSchema } from '@/lib/validation'
import { getSupabaseAdmin } from '@/lib/supabase-admin'
import { parseJsonBody } from '@/lib/parse-body'

const schema = unoActionSchema.pick({ gameId: true })

// System/timer route: any client may poke it, but it only acts once the turn
// deadline has genuinely passed (enforced in processUnoExpireTurn), so there's no
...
export async function POST(req: NextRequest) {
  ...
  const result = await processUnoExpireTurn(supabase, code)
