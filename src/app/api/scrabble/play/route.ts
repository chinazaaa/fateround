...
export async function POST(req: NextRequest) {
  const { data: body, error: bodyError } = await parseJsonBody(req, scrabblePlaySchema)
  if (bodyError) return bodyError

  const { gameId, resumeToken, tiles } = body
  ...
  const { error } = await processScrabblePlay(supabase, code, auth.player.id, tiles)
