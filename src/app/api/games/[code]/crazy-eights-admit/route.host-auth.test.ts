 ...
 * POST /api/games/[code]/crazy-eights-admit.
 *
...
vi.mock('@/lib/crazy-eights', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/crazy-eights')>()
  return { ...actual, admitCrazyEightsPlayer: async () => ({ error: null, status: 200 }) }
})
...
const post = (body: unknown) => POST(jsonRequest(`/api/games/${GAME_CODE}/crazy-eights-admit`, body), codeParams())

const STATUS_ERROR = 'Players can only be dealt in while the game is in progress'

describe('POST /api/games/[code]/crazy-eights-admit — host authorization', () => {
  ...
  it('rejects a non-Crazy-Eights game with 400 "Not a Crazy Eights game"', async () => {
    game = liveGame({ game_type: 'smash_marry_kill' })
    const res = await post({ hostToken: HOST_TOKEN, playerId: PLAYER_ID })
    expect(res.status).toBe(400)
    await expect(res.json()).resolves.toEqual({ error: 'Not a Crazy Eights game' })
  })

  it('rejects a non-active Crazy Eights game with the in-progress-only 400', async () => {
  ...
  it('authorizes the real host on an active Crazy Eights game and deals the player in', async () => {
    const res = await post({ hostToken: HOST_TOKEN, playerId: PLAYER_ID })
