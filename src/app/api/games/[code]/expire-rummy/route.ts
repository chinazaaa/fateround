...
import { parseGameType, isRummyGame } from '@/lib/game-types'
import { finishExpiredRummyGame, rummyGameSessionExpired } from '@/lib/rummy'
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (!rummyGameSessionExpired(game.session_started_at, game.game_duration_seconds)) {
    return NextResponse.json({ expired: false, finished: false })
  }

  const finished = await finishExpiredRummyGame(supabase, game)
