...
import { parseGameType, isWordSearchGame } from '@/lib/game-types'
import { finishExpiredWordSearchGame } from '@/lib/word-search-finish'
import { wordSearchGameSessionExpired } from '@/lib/word-search'
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (!isWordSearchGame(parseGameType(game.game_type))) {
    return NextResponse.json({ error: 'Not a Word Search game' }, { status: 400 })
  }
  ...
  const finished = await finishExpiredWordSearchGame(supabase, game)
