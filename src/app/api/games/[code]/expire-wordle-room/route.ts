...
import { parseGameType, isWordleRoomGame } from '@/lib/game-types'
import { WORDLE_ROOM_EXPIRE_GRACE_MS, finishExpiredWordleRoomGame, wordleRoomSessionExpired } from '@/lib/wordle-room'
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (!isWordleRoomGame(parseGameType(game.game_type))) {
    return NextResponse.json({ error: 'Not a Wordle Room game' }, { status: 400 })
  }
  if (game.status !== 'active') {
    return NextResponse.json({ expired: false, finished: game.status === 'finished' })
  }
  if (!wordleRoomSessionExpired(game.session_started_at, game.timer_seconds, WORDLE_ROOM_EXPIRE_GRACE_MS)) {
    return NextResponse.json({ expired: false, finished: false })
  }

  const finished = await finishExpiredWordleRoomGame(supabase, game, {
    graceMs: WORDLE_ROOM_EXPIRE_GRACE_MS,
  })
