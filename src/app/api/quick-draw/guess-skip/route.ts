...
import { processQuickDrawGuessSkip } from '@/lib/quick-draw-guess'
import { quickDrawGuessSkipSchema } from '@/lib/validation'
import { getSupabaseAdmin } from '@/lib/supabase-admin'
import { assertPlayer } from '@/lib/game-admin'
import { parseJsonBody } from '@/lib/parse-body'

export async function POST(req: NextRequest) {
  const { data, error: bodyError } = await parseJsonBody(req, quickDrawGuessSkipSchema)
  ...
  if (auth.player.spectator === true || auth.player.is_eliminated === true) {
    return NextResponse.json({ error: 'Spectators cannot skip words' }, { status: 403 })
  }

  const { error, internal } = await processQuickDrawGuessSkip(supabase, code, auth.player.id)
