...
import { processQuickDrawGuessGuess } from '@/lib/quick-draw-guess'
import { quickDrawGuessSchema } from '@/lib/validation'
import { getSupabaseAdmin } from '@/lib/supabase-admin'
import { assertPlayer } from '@/lib/game-admin'
import { parseJsonBody } from '@/lib/parse-body'

export async function POST(req: NextRequest) {
  const { data, error: bodyError } = await parseJsonBody(req, quickDrawGuessSchema)
  ...
  const { error, correct, internal } = await processQuickDrawGuessGuess(supabase, code, auth.player.id, text)
