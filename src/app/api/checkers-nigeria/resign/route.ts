...
import { processDraughts10Resign } from '@/lib/draughts10'
import { checkersResignSchema } from '@/lib/validation'
import { getSupabaseAdmin } from '@/lib/supabase-admin'
import { assertPlayer } from '@/lib/game-admin'
import { parseJsonBody } from '@/lib/parse-body'

export async function POST(req: NextRequest) {
  const { data: body, error: bodyError } = await parseJsonBody(req, checkersResignSchema)
  ...
  const { error } = await processDraughts10Resign(supabase, code, auth.player.id)
