...
import { processCrazyEightsDraw } from '@/lib/crazy-eights'
import { crazyEightsDrawSchema } from '@/lib/validation'
...
export async function POST(req: NextRequest) {
  const { data: body, error: bodyError } = await parseJsonBody(req, crazyEightsDrawSchema)
  ...
  const { error } = await processCrazyEightsDraw(supabase, code, auth.player.id)
