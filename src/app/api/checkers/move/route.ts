...
import { processCheckersMove } from '@/lib/checkers'
import { checkersMoveSchema } from '@/lib/validation'
...
export async function POST(req: NextRequest) {
  const { data: body, error: bodyError } = await parseJsonBody(req, checkersMoveSchema)
  ...
  const { error } = await processCheckersMove(supabase, code, auth.player.id, { from, to })
