...
import { processYahtzeeRoll } from '@/lib/yahtzee'
...
export async function POST(req: NextRequest) {
  const { data: body, error: bodyError } = await parseJsonBody(req, yahtzeeRollSchema)
  ...
  if (!isYahtzeeGame(parseGameType(game.game_type))) {
    return NextResponse.json({ error: 'Not a Five Dice game' }, { status: 400 })
  }
  ...
  const { error } = await processYahtzeeRoll(supabase, code, auth.player.id)
