...
/**
 * Pins the `games` read POST /api/mahjong/next-hand actually issues.
 *
...
// The hand-advance engine is out of scope here, as in the host-auth suite.
...
describe('POST /api/mahjong/next-hand — games select shape', () => {
  ...
  it('carries `game_type` through the projection, so the type gate still sees it', async () => {
    const ok = await post({ gameId: GAME_CODE, hostToken: HOST_TOKEN })
    expect(ok.status).toBe(200)
    await expect(ok.json()).resolves.toEqual({ success: true })

    game = gameRow({ status: 'active', game_type: 'smash_marry_kill' })
    const wrongType = await post({ gameId: GAME_CODE, hostToken: HOST_TOKEN })
