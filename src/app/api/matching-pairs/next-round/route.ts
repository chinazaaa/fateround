...
import { assertHostPlayerRemove } from '@/lib/game-admin'
...
export async function POST(req: NextRequest) {
  ...
  // Verify host.
  const host = await assertHostPlayerRemove(supabase, gameId, body.hostToken)
  if (host.error) return NextResponse.json({ error: host.error }, { status: host.status })

  // Load game to get round config and player list.
  const { data: game } = await supabase.from('games').select(GAME_SELECT).eq('id', gameId).maybeSingle()
  ...
  const { data: playersData } = await supabase.from('players').select('id').eq('game_id', gameId).eq('spectator', false)
  ...
  if (progressError) {
    return NextResponse.json(
      { error: internalErrorMessage('matching-pairs/next-round', progressError) },
      { status: 500 }
    )
  }
  ...
  if (pointerError) {
    return NextResponse.json(
      { error: internalErrorMessage('matching-pairs/next-round', pointerError) },
      { status: 500 }
    )
  }
