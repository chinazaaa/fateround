'use client'

/**
 * Solo Checkers vs Bot — client shell.
 *
 * Same state model as the other /play-solo/<game> clients:
 *  - the pure engine in `checkers-solo.ts` owns everything
 *  - `state` is `null` until mount to keep SSR/hydration in sync
 *  - after every hop that hands the turn to (or keeps it on) the bot, an
 *    effect fires the bot's next hop on a short timeout so multi-jumps play
 *    out visibly instead of instantly
 *  - sessionStorage keeps the game across a page reload
 */

import Link from 'next/link'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { CheckersGamePanel } from '@/components/checkers/CheckersBoard'
import {
  CHECKERS_SOLO_BOT_ID,
  CHECKERS_SOLO_HUMAN_ID,
  botColor,
  checkersSoloMove,
  humanColor,
  initCheckersSolo,
  isBotTurn,
  isHumanTurn,
  type CheckersSoloState,
} from '@/lib/checkers-solo'
import { pickCheckersBotMove, type CheckersBotDifficulty } from '@/lib/checkers-bot'
import { logSoloPlayFinished, logSoloPlayStarted, resetSoloSessionId, soloSessionId } from '@/lib/solo-play'
import { readSoloScoreboard, recordSoloOutcome, resetSoloScoreboard, type SoloScoreboard } from '@/lib/solo-scoreboard'
import { SoloScoreboardRow } from '@/components/solo/SoloScoreboardRow'
import type { Player } from '@/types'

const STORAGE_KEY = 'solo-checkers-state-v1'
const DIFFICULTY_KEY = 'solo-checkers-difficulty-v1'
const BOT_THINK_MS = 550

function loadPersistedState(): CheckersSoloState | null {
  if (typeof window === 'undefined') return null
  try {
    const raw = window.sessionStorage.getItem(STORAGE_KEY)
    if (!raw) return null
    const parsed = JSON.parse(raw) as CheckersSoloState
    if (!parsed?.session?.board || typeof parsed.session.board !== 'string') return null
    return parsed
  } catch {
    return null
  }
}

function persistState(state: CheckersSoloState): void {
  if (typeof window === 'undefined') return
  try {
    window.sessionStorage.setItem(STORAGE_KEY, JSON.stringify(state))
  } catch {
    /* noop */
  }
}

function clearPersistedState(): void {
  if (typeof window === 'undefined') return
  try {
    window.sessionStorage.removeItem(STORAGE_KEY)
  } catch {
    /* noop */
  }
}

function loadDifficulty(): CheckersBotDifficulty {
  if (typeof window === 'undefined') return 'normal'
  try {
    const raw = window.localStorage.getItem(DIFFICULTY_KEY)
    return raw === 'easy' || raw === 'hard' ? raw : 'normal'
  } catch {
    return 'normal'
  }
}

export function SoloCheckersClient() {
  const [state, setState] = useState<CheckersSoloState | null>(null)
  const [difficulty, setDifficulty] = useState<CheckersBotDifficulty>('normal')
  const [scoreboard, setScoreboard] = useState<SoloScoreboard>({ human: 0, bot: 0, draws: 0 })
  const stateRef = useRef<CheckersSoloState | null>(null)
  stateRef.current = state
  const scoredRef = useRef(false)
  const finishRef = useRef<HTMLDivElement | null>(null)

  useEffect(() => {
    const persisted = loadPersistedState()
    const d = loadDifficulty()
    setDifficulty(d)
    setState(persisted ?? initCheckersSolo())
    setScoreboard(readSoloScoreboard('checkers'))
    if (persisted && persisted.outcome != null) scoredRef.current = true
    if (!persisted) logSoloPlayStarted('checkers', d)
  }, [])

  useEffect(() => {
    if (!state || state.outcome == null || scoredRef.current) return
    const outcome = state.outcome === 'human' ? 'human' : state.outcome === 'draw' ? 'draw' : 'bot'
    setScoreboard(recordSoloOutcome('checkers', outcome))
    logSoloPlayFinished({
      gameType: 'checkers',
      outcome,
      sessionId: soloSessionId('checkers'),
      difficulty,
    })
    scoredRef.current = true
  }, [state, difficulty])

  useEffect(() => {
    if (state) persistState(state)
  }, [state])

  useEffect(() => {
    if (!state || state.outcome == null) return
    finishRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' })
  }, [state?.outcome])

  const setDifficultyStored = useCallback((d: CheckersBotDifficulty) => {
    setDifficulty(d)
    try {
      window.localStorage.setItem(DIFFICULTY_KEY, d)
    } catch {
      /* noop */
    }
  }, [])

  // Bot loop: whenever it's the bot's turn (including mid-chain jumps), pick
  // and apply the next hop after a short delay. The effect re-runs after each
  // hop because state changes, so a multi-jump plays out one visible step at
  // a time.
  useEffect(() => {
    if (!state || state.outcome != null) return
    if (!isBotTurn(state)) return
    const t = setTimeout(() => {
      const now = stateRef.current
      if (!now || !isBotTurn(now)) return
      const step = pickCheckersBotMove(now, difficulty)
      if (!step) return
      const r = checkersSoloMove(now, botColor(now), step.from, step.to)
      if (!r.error) setState(r.state)
    }, BOT_THINK_MS)
    return () => clearTimeout(t)
  }, [state, difficulty])

  const onMove = useCallback((from: string, to: string) => {
    const now = stateRef.current
    if (!now) return
    if (!isHumanTurn(now)) return
    const r = checkersSoloMove(now, humanColor(now), from, to)
    if (!r.error) setState(r.state)
  }, [])

  const onResign = useCallback(() => {
    const now = stateRef.current
    if (!now || now.outcome != null) return
    const hc = humanColor(now)
    const bc = botColor(now)
    setState({
      ...now,
      outcome: 'bot',
      session: {
        ...now.session,
        status: 'finished',
        result_reason: 'resignation',
        winner_player_id: CHECKERS_SOLO_BOT_ID,
        is_draw: false,
        status_message: 'You resigned — Bot wins!',
        turn_deadline_at: null,
      },
    })
    // Silence unused warnings for helpers kept to make intent explicit.
    void hc
    void bc
  }, [])

  const restart = useCallback(() => {
    clearPersistedState()
    setState(initCheckersSolo())
    scoredRef.current = false
    resetSoloSessionId('checkers')
    logSoloPlayStarted('checkers', difficulty)
  }, [difficulty])

  const resetScore = useCallback(() => {
    setScoreboard(resetSoloScoreboard('checkers'))
  }, [])

  const players = useMemo<Player[]>(() => {
    if (!state) return []
    const hc = humanColor(state)
    const humanIsRed = hc === 'r'
    return [
      { id: CHECKERS_SOLO_HUMAN_ID, name: 'You' },
      { id: CHECKERS_SOLO_BOT_ID, name: `Bot (${difficulty})` },
    ].map((p) => ({ ...p, __colorHint: humanIsRed })) as unknown as Player[]
  }, [state, difficulty])

  if (!state) {
    return (
      <div className="fr-room fr-room-phone">
        <div className="p-6 text-center text-muted text-sm">Setting up the board…</div>
      </div>
    )
  }

  const finished = state.outcome != null
  const humanWon = state.outcome === 'human'
  const draw = state.outcome === 'draw'
  const myTurn = isHumanTurn(state)

  return (
    <div className="fr-room fr-room-phone">
      <div className="flex items-center justify-between gap-3 border-b border-[var(--border)] px-4 py-3">
        <div className="min-w-0">
          <h1 className="text-sm font-bold text-body">Checkers — solo vs bot</h1>
          <p className="text-faint text-xs">Practice mode · no room, no account</p>
        </div>
        <div className="flex items-center gap-2 text-xs">
          <label className="text-muted">
            Difficulty
            <select
              className="input-field ml-2 py-1 text-xs"
              value={difficulty}
              onChange={(e) => setDifficultyStored(e.target.value as CheckersBotDifficulty)}
            >
              <option value="easy">Easy</option>
              <option value="normal">Normal</option>
              <option value="hard">Hard</option>
            </select>
          </label>
          <button type="button" onClick={restart} className="btn-secondary text-xs">
            New game
          </button>
        </div>
      </div>

      <div className="px-3 py-4">
        <CheckersGamePanel
          session={state.session}
          players={players}
          myPlayerId={CHECKERS_SOLO_HUMAN_ID}
          isMyTurn={myTurn}
          onMove={myTurn ? onMove : undefined}
          onResign={!finished ? onResign : undefined}
        />
      </div>

      {finished && (
        <div
          ref={finishRef}
          className="mx-3 my-4 rounded-2xl border border-[var(--border)] bg-[var(--surface-inset-bg)] p-5 text-center"
        >
          <p className="text-lg font-black text-body">
            {humanWon ? 'You won ' : draw ? "It's a draw" : 'Bot wins'}
            {humanWon && <span aria-hidden> 🎉</span>}
          </p>
          <p className="text-muted mt-1 text-sm">Practice mode — no ranking, just for fun.</p>
          <SoloScoreboardRow scoreboard={scoreboard} onReset={resetScore} />
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-center">
            <button type="button" onClick={restart} className="btn-primary">
              Play again
            </button>
            <Link href="/create?type=checkers" className="btn-secondary text-center">
              Start a real room
            </Link>
            <Link href="/play-solo" className="btn-secondary text-center">
              Play other solo games
            </Link>
          </div>
        </div>
      )}
    </div>
  )
}
