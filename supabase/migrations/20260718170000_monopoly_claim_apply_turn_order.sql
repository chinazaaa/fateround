-- Fix: Monopoly player removal fails with "Something went wrong".
--
-- removeMonopolyPlayer reorders the turn queue and sends `turn_order` in the board
-- patch to monopoly_claim_and_apply. That function whitelists the keys it applies
-- and RAISE EXCEPTIONs on any it doesn't recognise — and `turn_order` was never in
-- the list. So every leave/remove during an ACTIVE game raised UNKNOWN_BOARD_COLUMN,
-- which the caller surfaced as the generic error. (Game-start and late-join write
-- turn_order with a direct UPDATE, so only removal hit this path.)
--
-- Add `turn_order` (uuid[]) to both the accepted-key whitelist and the UPDATE. The
-- patch carries it as a JSON array of uuid strings, converted back to uuid[] here.
...
BEGIN
  FOR v_key IN SELECT jsonb_object_keys(coalesce(p_board_patch, '{}'::jsonb)) LOOP
    IF v_key NOT IN (
      'phase', 'last_dice', 'consecutive_doubles', 'current_turn_index', 'turn_order',
  ...
  UPDATE monopoly_boards b SET
    ...
    turn_order          = CASE WHEN p_board_patch ? 'turn_order'          THEN ARRAY(SELECT jsonb_array_elements_text(p_board_patch->'turn_order'))::uuid[] ELSE b.turn_order END,
