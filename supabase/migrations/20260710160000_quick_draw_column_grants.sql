-- Re-grant column-level SELECT on games/players after quick_draw_* columns were added.
--
-- Migration 0122 grants anon/authenticated SELECT on named columns only (not table-wide).
-- New games columns must re-run this block or client reads error with
-- "permission denied for table games" (42501) — including Quick Draw host/player flows
-- that load GAME_SELECT with quick_draw_variant, quick_draw_play_mode, quick_draw_num_teams.
