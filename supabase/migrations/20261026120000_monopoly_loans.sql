-- Estate Kings (Monopoly) Loan Facilities Migration
-- Adds loans column to monopoly_boards and loan facility game settings to games.

-- 1. Add loans array to monopoly_boards
ALTER TABLE public.monopoly_boards
  ADD COLUMN IF NOT EXISTS loans jsonb NOT NULL DEFAULT '[]';

-- 2. Add game settings on games table
ALTER TABLE public.games
  ADD COLUMN IF NOT EXISTS monopoly_loans_enabled boolean DEFAULT true,
  ADD COLUMN IF NOT EXISTS monopoly_loan_interest integer DEFAULT 15,
  ADD COLUMN IF NOT EXISTS monopoly_loan_term_rounds integer DEFAULT 4;

-- 3. Re-grant column-level select on games table to anon and authenticated
DO $$
DECLARE
  game_cols text;
  role_name text;
...
BEGIN
  ...
  UPDATE monopoly_boards b SET
    ...
    loans               = CASE WHEN p_board_patch ? 'loans'               THEN p_board_patch->'loans'                                     ELSE b.loans               END,
    updated_at          = now()
