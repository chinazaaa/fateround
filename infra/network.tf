...
# Private subnets host the app instances (no direct inbound from the internet).
...
# Single NAT gateway (cost trade-off: one NAT, not one per AZ). Instances use it
# for outbound (ECR pulls, SSM, Supabase). Bump to one-per-AZ for HA if needed.
